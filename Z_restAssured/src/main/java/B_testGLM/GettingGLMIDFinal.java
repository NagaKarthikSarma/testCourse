...
public class GettingGLMIDFinal extends Thread {
	...
	public static List<Integer> nullRow= new ArrayList<Integer>();
	public GettingGLMIDFinal(int i, CountDownLatch latch) {
		this.row = i;
		this.latch = latch;
	}

	public GettingGLMIDFinal() {
...
//System.out.println("*****************************************   "+endFile+ "  ----"+file);
                 ...
                 int latchSize = (endFile - startFile ) +1;
				 
				 CountDownLatch	latch = new CountDownLatch(latchSize);
						for (int i = startFile; i <=endFile; i++) {
							 ex = new GettingGLMIDFinal(i, latch);
							ex.start();	
...
System.out.println("***************************************** startfile:[   "+startFile+ " ]  --------- endfile:[  " +endFile+"  ]");
						startFile = endFile+1;
...
// GettingGLMID ex1g= null;

// if ((nullRow.size()-1)>0){
// 	latch = new CountDownLatch(nullRow.size()-1);
// 	CountDownLatch latchAgain = new CountDownLatch(nullRow.size()-1);
	
// 	for (int i : nullRow){

// 	 ex1g= new GettingGLMID(i,latch);
// 		ex1g.start();
...
long endTime = System.currentTimeMillis();	
	...
	//	System.out.println("------------------------------------------------------ "+  (endTime-startTime) + " MilliSeconds  -----------------");
		...
		System.out.println();
		GettingGLMIDFinal gd = new GettingGLMIDFinal();
	...
	public void run() {
		...
		siteID = getSiteIdFromGLMAPI(payload, streetAddress,row);

		mapValues.put(Integer.valueOf(i), siteID);
		latch.countDown();
		
		
	}

	public static List<String> getSiteIdFromGLMAPI(String payload, String streetAddress,int row) {
...
//System.out.println(extractor.getString("resultObject[0].locationProductInfos[0].productName"));
		...
		if (siteId ==null){
			nullRow.add(Integer.valueOf(row));
		}
		return dummy;
	...
	public void duplicateAddress() throws IOException, CsvException{
		...
		for (int i = 1; i < records.size(); i++) {
			...
			if (csvBody.get(i)[8].equals("Duplicate")){
				cnt = cnt + 1;
				int defaultValue=0;
		repeatedValues.put(address, Integer.valueOf(repeatedValues.getOrDefault(address, Integer.valueOf(0)) + 1));
			}
...
//  }
	...
	public void fallout() {
		...
		Set<String> notValid = new HashSet<String>();
		...
		if (notValid.size() > 0) {
			...
			System.out.println();
			System.out.println("Total fallout locations:  " + notValid.size());
