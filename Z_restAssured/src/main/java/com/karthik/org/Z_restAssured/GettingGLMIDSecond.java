...
public class GettingGLMIDSecond extends Thread {
	...
	public GettingGLMIDSecond(int i, CountDownLatch latch) {
		this.row = i;
		this.latch = latch;
	}

	public GettingGLMIDSecond() {
	...
	public static void main(String[] args) throws IOException, InterruptedException, CsvException {
		...
		for (int i = rowIndex; i < records.size(); i++) {
			GettingGLMIDSecond ex = new GettingGLMIDSecond(rowIndex, latch);
			ex.start();
			rowIndex++;
		}
		...
		System.out.println();
		GettingGLMIDSecond gd = new GettingGLMIDSecond();
